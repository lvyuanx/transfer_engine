...
def test_share_download_file(tmp_path):
    seed(tmp_path / "shared")
    with TestClient(make_app(tmp_path)) as client:
        sid = create_share(client, "a.txt")
        resp = client.get(f"/s/{sid}/download")
...
def test_share_page_renders(tmp_path):
    seed(tmp_path / "shared")
    with TestClient(make_app(tmp_path)) as client:
        sid = create_share(client, "a.txt")
        resp = client.get(f"/s/{sid}")
        assert resp.status_code == 200
        assert "a.txt" in resp.text
        assert "download" in resp.text


def test_share_encrypted_requires_password(tmp_path):
    seed(tmp_path / "shared")
    with TestClient(make_app(tmp_path)) as client:
        sid = create_share(client, "a.txt", encrypted=True, password="pw")
        assert client.get(f"/s/{sid}/download").status_code == 403
        assert client.get(f"/s/{sid}/download", params={"password": "bad"}).status_code == 403
        assert client.get(f"/s/{sid}/download", params={"password": "pw"}).status_code == 200
...
def test_share_expired_returns_404(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        assert client.get(f"/s/{sid}/download").status_code == 404
...
def test_share_vault_bypass_requires_token(tmp_path):
    ...
    with TestClient(make_app(tmp_path)) as client:
        ...
        assert client.get(f"/s/{sid}/download").status_code == 200
